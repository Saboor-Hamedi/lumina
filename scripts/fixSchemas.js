...
const files = fs.readdirSync(dir).filter(f => f.endsWith('.js') && f !== 'index.js' && f !== 'bulkExecutor.js' && f !== 'executeBulkPlan.js' && f !== 'LuminaChat.js');
...
for (const file of files) {
  ...
  if (content.includes('aiSdk.jsonSchema(')) {
    ...
    const propMatch = schemaStr.match(/properties:\s*\{([\s\S]*?)\}\s*(?:,\s*required:\s*\[(.*?)\])?/);
    if (propMatch) {
      const propsStr = propMatch[1];
      const reqStr = propMatch[2] || '';
      const required = reqStr.split(',').map(s => s.trim().replace(/['"]/g, '')).filter(Boolean);
      
      const newProps = [];
      const lines = propsStr.split('\n');
      for (const line of lines) {
        const m = line.match(/^\s*([a-zA-Z0-9_]+)\s*:\s*\{\s*type:\s*['"]([a-zA-Z]+)['"],\s*description:\s*['"](.*?)['"]\s*\}/);
        if (m) {
          const propName = m[1];
          const type = m[2];
          const desc = m[3];
          const isReq = required.includes(propName);
          let zType = type === 'string' ? 'z.string()' : type === 'number' ? 'z.number()' : type === 'boolean' ? 'z.boolean()' : 'z.any()';
          newProps.push(`      ${propName}: ${zType}.describe('${desc.replace(/'/g, "\\'")}')${isReq ? '' : '.optional()'}`);
        }
      }
