...
export class VaultOperations {
    ...
    if (oldSnippet) {
      ...
      if (oldFileName !== newFileName || oldRelativeFolder !== relativeFolder) {
        ...
        try {
          ...
          console.warn('[VaultOperations] Warning: Could not delete old file:', oldFileName, err.message)
        }
      }
    }
    ...
    await Promise.all(
      folderIds.map(async (folderPath) => {
        try {
          ...
          console.warn('[VaultOperations] Bulk folder delete warning:', folderPath, err.message)
        }
      })
    )
    ...
    await Promise.all(
      snippetIds.map(async (id) => {
        try {
          ...
          console.warn('[VaultOperations] Bulk note delete warning:', id, err.message)
        }
      })
    )
  ...
  static async moveFile(vaultPath, oldRelPath, newRelPath) {
    ...
    try {
      ...
      console.error('[VaultOperations] Move file failed:', err)
      throw err
    }
  }
  ...
  static async importExternalPaths(vaultPath, foldersSet, sourcePaths = [], targetFolderId = '') {
    ...
    for (const srcPath of sourcePaths) {
      try {
        ...
        console.error('[VaultOperations] Error importing path:', srcPath, err)
      }
    }
