...
class WorkspaceSearch {
  ...
  async init(userDataPath) {
    ...
    console.info('[WorkspaceSearch] ✓ Initialized (embedder deferred)')
  ...
  async _getEmbedder() {
    ...
    try {
      console.info('[WorkspaceSearch] Lazy-loading embedder model...')
  ...
  async loadIndex() {
    try {
      ...
      this.isLoaded = true
      console.info(`[WorkspaceSearch] ✓ Loaded ${this.index.length} chunks into memory`)
    } catch (err) {
      console.error('[WorkspaceSearch] Failed to load index:', err)
      this.index = []
      this.embeddingsBuffer = null
      this.isLoaded = true
    }
  ...
  getChunkEmbedding(chunk) {
    ...
    if (requiredSize > this.embeddingsBuffer.length) {
      console.warn(
        `[WorkspaceSearch] Chunk embedding out of bounds: offset=${offset}, required=${requiredSize}, buffer=${this.embeddingsBuffer.length}`
      )
      return null
    }
  ...
  async search(query, options = {}) {
    ...
    try {
      ...
      console.error('[WorkspaceSearch] Search failed:', err)
      return []
    }
  }
...
export default new WorkspaceSearch()
