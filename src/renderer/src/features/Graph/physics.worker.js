  ...
  if (type === 'INIT') {
    ...
    simulation = forceSimulation(nodes)
      ...
      .force(
        'custom_charge',
        forceManyBody()
          .strength(-2000 * payload.settings.repelForce)
          .distanceMax(4000)
      )
      // Prevent overlapping
      .force(
      ...
      .force(
        ...
        forceLink(links)
          .id((d) => d.id)
          .distance(250)
          .strength(payload.settings.linkForce * 2)
      )
    ...
    simulation.force('custom_x').strength(payload.settings.centerForce * 0.005)
    simulation.force('custom_y').strength(payload.settings.centerForce * 0.005)
