...
// Unread: 0, In Progress: 1, Completed: 2
...
export function toggleNoteStatus(trackName, noteId) {
  ...
  const next = current === STATUS_UNREAD ? STATUS_IN_PROGRESS : 
               current === STATUS_IN_PROGRESS ? STATUS_COMPLETED : 
               STATUS_UNREAD
...
/**
 * Calculates overall roadmap progress for an entire markdown document.
 * Scans for headings and roadmap tables with `#` / `No.` / `Status` columns.
 */
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (line.startsWith('|') && line.endsWith('|')) {
      ...
      if (!inTable) {
        ...
        tableHeader = cells
        hashColIdx = cells.findIndex(c => /^(#|no\.?|status)$/i.test(c.replace(/[*_`]/g, '').trim()))
        inTable = true
        continue
      }
