...
const MarkdownEditor = React.memo(
  ({ snippet, onSave, onToggleInspector }) => {
    ...
    const createTargetState = useCallback(
      (content, sel) => {
        return EditorState.create({
          doc: content || '',
          extensions: [
            // Use a compartment for line numbers
            lineNumberCompartment.current.of(settings.showLineNumbers ? lineNumbers() : []),
            highlightActiveLine(),
            dropCursor(),
            history(),
            indentOnInput(),
            ...
            EditorView.updateListener.of((update) => {
              ...
              if (update.docChanged) {
                ...
                // Scroll to top if content becomes very short (e.g., after deleting all content)
              ...
              if (update.selectionSet) {
                ...
                updateSnippetSelection(snippetRef.current?.id, {
                  anchor: sel.anchor,
                  head: sel.head
                })
              }
      ...
      [onSave, setSelectedSnippet, updateSnippetSelection]
    )

    // No need for preview content update - reading preview shows rendered HTML
    ...
    useEffect(() => {
      ...
      setIsDirty(false)
    }, [snippet?.id, snippet?.title, createTargetState]) // Update when title changes too

    // Highlight code blocks in reading/overlay previews when previewContent changes
    useEffect(() => {
      try {
        const inDocCodes = document.querySelectorAll(
          '.reading-preview-container pre code, .reading-preview-content pre code, .in-editor-preview-card pre code'
        )
    ...
    useEffect(() => {
      ...
      const getVars = () => {
        const w =
          getComputedStyle(root).getPropertyValue('--caret-width').trim() || caretWidth || '2px'
        const c =
          getComputedStyle(root).getPropertyValue('--caret-color').trim() ||
          caretColor ||
          'var(--text-accent)'
        const style =
        ...
        try {
          ...
          const cursors = [
            ...
            ...(document.querySelectorAll(
              '.markdown-editor .cm-cursor, .markdown-editor .cm-cursor-primary'
            ) || [])
          ]
      ...
      if (viewRef.current.contentDOM)
        mo.observe(viewRef.current.contentDOM, {
          childList: true,
          subtree: true,
          attributes: true,
    ...
    useEffect(() => {
      ...
      try {
        ...
        console.error('[MarkdownEditor] Theme application failed:', err)
    ...
    // Reconfigure line numbers compartment when setting changes
    useEffect(() => {
      if (!viewRef.current) return
      const ext = settings?.showLineNumbers ? lineNumbers() : []
      try {
    ...
    }, [settings?.showLineNumbers, createTargetState])
    ...
    const handleInlineAIInsert = useCallback((text) => {
      ...
      try {
        ...
        const transaction = view.state.update({
          ...
          selection: {
            anchor: Math.min(from + String(text).length, docLength + String(text).length)
          }
        })
    ...
    useEffect(() => {
      ...
      const handleClick = (e) => {
        ...
        if (wikiLink) {
          const titleAttr =
            wikiLink.getAttribute('data-title') || wikiLink.getAttribute('data-target')
          if (titleAttr) {
    ...
    // Container class for mode-source
    const safeViewMode = 'source'
    ...
    return (
      <div className={`markdown-editor mode-${safeViewMode} cursor-${caretStyle || 'smooth'}`}>
        ...
        <div
          ...
          ref={scrollerRef}
          style={{ position: 'relative', height: '100%', flex: 1, overflowY: 'auto' }}
        >
          <div className="editor-canvas-wrap" style={{ position: 'relative' }}>
            {/* Editor: Show CodeMirror */}
            ...
            {isOverlayOpen && (
              <div className="in-editor-preview-overlay" onClick={(e) => e.stopPropagation()}>
                <div className="in-editor-preview-card" role="dialog" aria-modal="true">
                  ...
                  <div
                    className="in-editor-preview-body seamless-scrollbar"
                    style={{ overflow: 'auto' }}
                  >
                    <div className="preview-inner-canvas">
                      <style>{
                        /* reuse modal preview styles inline for parity */ `
        ...
        <EditorFooter isDirty={isDirty} onTogglePreview={() => setIsPreviewOpen((p) => !p)} />
