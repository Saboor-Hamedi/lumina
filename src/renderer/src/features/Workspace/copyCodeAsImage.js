...
export async function copyCodeAsImage(code, lang = 'CODE') {
  ...
  return new Promise((resolve, reject) => {
    try {
      ...
      let maxLineLength = lang.length + 10
      ...
      // Header Bar Window Buttons (macOS dots)
      const dotY = cardY + 18
      const dotRadius = 5.5
      ...
      for (let i = 0; i < lines.length; i++) {
        const line = lines[i]
        // Syntax tint heuristics for comments / keywords
        if (line.trim().startsWith('//') || line.trim().startsWith('#')) {
          ctx.fillStyle = '#6a9955'
        } else if (line.trim().startsWith('import ') || line.trim().startsWith('export ') || line.trim().startsWith('const ') || line.trim().startsWith('function ')) {
