...
const getIconColor = (iconType, title) => {
  ...
  const cacheKey = `${iconType?.name || 'unknown'}:${titleLower}`

  if (_colorCache.has(cacheKey)) return _colorCache.get(cacheKey)

  let color = 'var(--icon-primary, var(--text-accent))'
    ...
    if (LucideIcons[snippet.customIcon]) {
      ...
      return (
        <span
          className={className}
          style={{
            fontSize: size * 1.2, // Slightly larger to match visual weight of SVG icons
            lineHeight: 1,
            display: 'inline-flex',
            alignItems: 'center',
            justifyContent: 'center',
