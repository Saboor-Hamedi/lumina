...
const IndexingStatus = () => {
  ...
  return (
    <div 
      ...
      style={{ bottom: '24px', top: 'auto', paddingBottom: !isComplete ? '10px' : '8px', zIndex: 10001, width: '280px' }}
    >
      <div className="indexing-toast-content" style={{ alignItems: 'center' }}>
        <div className="indexing-icon-wrapper" style={isComplete ? { background: 'rgba(34, 197, 94, 0.14)', color: '#22c55e' } : {}}>
          {isComplete ? (
        ...
        <div className="indexing-toast-message" style={{ display: 'flex', flexDirection: 'column' }}>
          ...
          <div style={{ fontSize: '10.5px', color: 'var(--text-faint)', fontWeight: 400, marginTop: '2px', lineHeight: 1.2, fontVariantNumeric: 'tabular-nums' }}>
        ...
        {!isComplete && (
          <span style={{ fontSize: '11.5px', fontWeight: 600, color: 'var(--text-faint)', marginLeft: '8px', fontVariantNumeric: 'tabular-nums' }}>
      ...
      {!isComplete && (
        <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, height: '3px', backgroundColor: 'var(--bg-active)' }}>
          <div style={{ height: '100%', backgroundColor: 'var(--text-accent)', width: `${safeProgress}%`, transition: 'width 0.3s ease-out' }} />
        </div>
      )}
    </div>
  )
